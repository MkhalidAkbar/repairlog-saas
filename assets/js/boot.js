// ====== GUARD PERUBAHAN BELUM DISIMPAN + MINI MODAL ======
let formDirty = false;
...
function requestCloseModal(id) {
  ...
  if (id === "formModal" && formDirty) {
    showMini(
      ...
      "Ada perubahan yang belum disimpan. Simpan dulu atau buang perubahan?",
      [
  ...
  if (id === "ownerModal" && ownerDirty) {
    showMini(
      "Perubahan belum disimpan",
      "Ada perubahan di Control Panel yang belum disimpan. Yakin keluar tanpa menyimpan?",
      [
        { label: "Batal", cls: "secondary", fn: null },
        {
          label: "Buang & keluar",
...
const TOUR_STEPS = [
  ...
  },
  {
    sel: "#notifBtn",
    title: "Notifikasi",
    text: "Titik merah muncul saat ada komentar baru pada laporan yang belum kamu baca.",
  },
  {
    sel: "nav .btn.small",
    title: "Tambah Servis",
...
function showTourStep() {
  ...
  $("tourNext").textContent =
    tourIdx >= TOUR_STEPS.length - 1 ? "Selesai" : "Lanjut →";
  const tip = $("tourTip");
...
// ====== LISENSI / KILL SWITCH (master control panel) ======
function lockScreen(judul, pesan) {
  document.body.innerHTML =
    '<div style="padding:48px;text-align:center;font-family:sans-serif"><h2>' +
    judul +
    "</h2><p>" +
    pesan +
    "</p></div>";
...
async function checkLicense() {
  if (!MASTER_URL || !MASTER_KEY || !STORE_ID) return true; // lisensi tidak dikonfigurasi → jangan kunci (versi Pro penuh)
  try {
    const lic = supabase.createClient(MASTER_URL, MASTER_KEY);
    const { data } = await lic.rpc("get_license", { p_slug: STORE_ID });
    const row = Array.isArray(data) ? data[0] : data;
    if (!row) return true; // toko belum terdaftar di master → jangan kunci
    if (row.status === "suspended") {
      lockScreen(
        "🔒 Layanan dinonaktifkan sementara",
        "Silakan hubungi penyedia layanan untuk mengaktifkan kembali.",
      );
      return false;
    }
    if (row.due_date) {
      ...
      if (new Date() > habis) {
        lockScreen(
          ...
          (row.status === "trial"
            ? "Masa uji coba (trial) sudah habis. "
    ...
    if (!FEATURES.whitelabel) {
      BRAND = { ...VENDOR_BRAND };
      applyBrand();
    } // tanpa white-label → brand vendor
    if (row.storage_limit_mb != null)
      LICENSE_STORAGE_MB = Number(row.storage_limit_mb);
    return true;
  } catch (e) {
    return true;
  } // gagal cek (offline dll) → biarkan toko tetap jalan
}

// ====== ABSENSI TEKNISI (fitur terpisah) ======
function fmtTime(t) {
  return t
...
async function renderAttend() {
  ...
  if (self) {
    ...
    const btns =
      !cur || !cur.check_in
        ? `<button class="btn small" onclick="attendCheckIn()">🟢 Check-in</button>`
        : cur.check_in && !cur.check_out
          ? `<button class="btn small danger" onclick="attendCheckOut()">🔴 Check-out</button>`
          : `<span class="pill selesai">✅ Absen hari ini selesai</span>`;
    self.innerHTML = `<div class="card"><div class="row"><h3 style="margin:0">👤 ${esc(ME.name || ME.email || "Saya")}</h3><span class="muted">${new Date().toLocaleDateString("id-ID", { weekday: "long", day: "2-digit", month: "long", year: "numeric" })}</span></div><div class="row" style="flex-wrap:wrap;gap:10px"><div>Masuk: <b>${inT}</b> &nbsp;•&nbsp; Pulang: <b>${outT}</b></div><div style="display:flex;gap:8px">${btns}</div></div></div>`;
  }
  const box = $("attendBox");
  if (!box) return;
  if (isOwner()) {
    ...
    const _months = [
      ...new Set(
        _all.map((a) => (a.work_date || "").slice(0, 7)).filter(Boolean),
      ),
    ]
      .sort()
      .reverse();
    ...
    const _d = _attMonth
      ? _all.filter((a) => (a.work_date || "").slice(0, 7) === _attMonth)
      : _all;
    if (!_d.length) {
    ...
    box.innerHTML =
      ...
      _pg
        .map(
          (a) =>
            `<tr><td>${fmtDate(a.work_date)}</td><td>${esc(a.user_name || "-")}</td><td>${fmtTime(a.check_in)}</td><td>${fmtTime(a.check_out)}</td></tr>`,
        )
        .join("") +
      `</table>` +
      attPager(_tp);
    ...
    const _pg = _d.slice((_attPage - 1) * _per, (_attPage - 1) * _per + _per);
    box.innerHTML =
      `<h3 style="margin:6px 0 10px">📋 Riwayat Absensi Saya</h3><table class="ftbl"><tr><th>Tanggal</th><th>Masuk</th><th>Pulang</th></tr>` +
      _pg
        .map(
          (a) =>
            `<tr><td>${fmtDate(a.work_date)}</td><td>${fmtTime(a.check_in)}</td><td>${fmtTime(a.check_out)}</td></tr>`,
        )
        .join("") +
      `</table>` +
      attPager(_tp);
...
async function boot() {
  await fetchConfig();
  if (!(await checkLicense())) return; // toko disuspend / trial habis → layar terkunci
