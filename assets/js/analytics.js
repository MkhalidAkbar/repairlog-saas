// ====== PRIORITY 12: BUSINESS ANALYTICS ======
...
function analyticsBarList(items, emptyText) {
  if (!items.length)
    return `<div class="business-empty">${esc(emptyText)}</div>`;
  const max = Math.max(...items.map((item) => Number(item.value) || 0), 1);
  return `<div class="analytics-bars">${items
    .map(
      (item) =>
        `<div class="analytics-bar-row"><div><strong>${esc(item.label)}</strong><span>${esc(item.meta || String(item.value))}</span></div><div class="analytics-bar-track"><span style="width:${Math.max(4, ((Number(item.value) || 0) / max) * 100)}%;background:${item.color || "var(--accent)"}"></span></div></div>`,
    )
    .join("")}</div>`;
}
...
function renderAnalytics() {
  ...
  const completed = list.filter((report) =>
    typeof isFinalized === "function"
      ? isFinalized(report)
      : /selesai|diambil/i.test(report.status || ""),
  );
  ...
  const avgDays = turnaround.length
    ? turnaround.reduce((sum, value) => sum + value, 0) / turnaround.length
    : 0;
  const slaRows = completed.filter((report) => report.sla_due_at);
  const slaOnTime = slaRows.filter(
    (report) => analyticsFinishDate(report) <= new Date(report.sla_due_at),
  ).length;
  const approvalRows = list.filter((report) =>
    ["approved", "rejected"].includes(report.approval_status),
  );
  const approved = approvalRows.filter(
    (report) => report.approval_status === "approved",
  ).length;
  const sourceIdsWithClaims = new Set(
    WARRANTY_CLAIMS.map((claim) => String(claim.source_report_id)),
  );
  const firstTimeFixed = completed.filter(
    (report) => !sourceIdsWithClaims.has(String(report.id)),
  ).length;
  const revenue = list.reduce(
    (sum, report) => sum + (Number(report.fee) || 0),
    0,
  );
  const cost = list.reduce(
    (sum, report) => sum + (Number(report.cost) || 0),
    0,
  );
  const margin = revenue - cost;
  const marginRate = revenue ? (margin / revenue) * 100 : 0;
  const claimsInRange = WARRANTY_CLAIMS.filter((claim) =>
    list.some((report) => String(report.id) === String(claim.source_report_id)),
  );
  const repeatRate = completed.length
    ? (claimsInRange.length / completed.length) * 100
    : 0;

  const devices = analyticsGroupCount(
    list,
    (report) => report.device_type || "Perangkat",
  );
  const brands = analyticsGroupCount(
    list,
    (report) => report.brand || "Tanpa merek",
  );
  const stages = analyticsGroupCount(
    list,
    (report) => report.stage || report.status,
  );
  ...
  list.forEach((report) =>
    (Array.isArray(report.cost_items) ? report.cost_items : []).forEach(
      (item) => {
        ...
        partMap.set(
          item.label,
          (partMap.get(item.label) || 0) + (Number(item.qty) || 1),
        );
      },
    ),
  );
  const topParts = [...partMap.entries()]
    ...
    .sort((a, b) => b.value - a.value)
    .slice(0, 6);
  const productivity = analyticsGroupCount(
    completed,
    (report) =>
      TEAM.find(
        (member) => String(member.user_id) === String(report.assigned_to),
...
function exportAnalyticsCsv() {
  ...
  const rows = [
    ...
    ...list.map((report) => [
      ...
      (Number(report.fee) || 0) - (Number(report.cost) || 0),
      report.sla_due_at || "",
      report.qc_status || "",
