// ====== WORKFLOW v3: SLA, PUSAT TINDAKAN, TIMELINE & PERSETUJUAN ======
const SLA_DAYS_BY_LEVEL = { 1: 1, 2: 3, 3: 5, 4: 7 };
...
function reportSlaDueAt(r) {
  ...
  const start = new Date(
    String(source).length <= 10 ? `${source}T09:00:00` : source,
  );
...
function slaState(r) {
  ...
  if (remainingMs < 0) {
    return { key: "overdue", label: "Melewati SLA", remainingMs, due };
  }
  if (remainingMs <= 86400000) {
    return { key: "soon", label: "Jatuh tempo <24 jam", remainingMs, due };
  }
  return { key: "ok", label: "Sesuai target", remainingMs, due };
}
...
function compactDuration(ms) {
  ...
  if (days > 0)
    return `${days} hari ${hours < 24 ? `${hours} jam` : ""}`.trim();
  return `${hours} jam`;
}
...
function slaDescription(r) {
  ...
  if (state.key === "overdue")
    return `Terlambat ${compactDuration(state.remainingMs)}`;
...
function workflowPayload() {
  return {
    sla_due_at: localDateTimeToIso(
      ($("f_sla_due") && $("f_sla_due").value) || "",
    ),
    delay_reason: (
      ($("f_delay_reason") && $("f_delay_reason").value) ||
      ""
    ).trim(),
  };
}
...
function actionSeverityRank(severity) {
  return { risk: 0, attention: 1, info: 2 }[severity] ?? 3;
}
...
function renderActionCenter(source) {
  ...
  const summary = [
    risk ? `<span class="action-summary risk">${risk} mendesak</span>` : "",
    attention
      ? `<span class="action-summary attention">${attention} perlu perhatian</span>`
      : "",
    info
      ? `<span class="action-summary info">${info} tindak lanjut</span>`
      : "",
  ]
    .filter(Boolean)
    .join("");
  const list = visible.length
    ? visible
        .map(
          (item) =>
            `<button type="button" class="action-item ${item.severity}" onclick="openActionItem('${item.type}','${item.id}')"><span class="action-mark" aria-hidden="true">${item.severity === "info" ? "i" : "!"}</span><span class="action-copy"><strong>${esc(item.title)}</strong><small>${esc(item.meta)}</small></span><span class="action-arrow" aria-hidden="true">›</span></button>`,
        )
        .join("")
    : `<div class="action-empty"><span aria-hidden="true">✓</span><div><strong>Tidak ada tindakan mendesak</strong><small>Semua pekerjaan berada dalam kondisi terkendali.</small></div></div>`;
...
function approvalMeta(r) {
  const status = String(
    (r && r.approval_status) || "not_requested",
  ).toLowerCase();
  const map = {
    pending: { label: "Menunggu pelanggan", className: "pending" },
    approved: { label: "Disetujui", className: "approved" },
    rejected: { label: "Ditolak", className: "rejected" },
    canceled: { label: "Dibatalkan", className: "neutral" },
    not_requested: { label: "Belum diminta", className: "neutral" },
  };
...
async function requestEstimateApproval(id) {
  ...
  if (amount <= 0) {
    toast(
      "Isi biaya servis terlebih dahulu sebelum meminta persetujuan.",
      "error",
    );
    return;
  }
  ...
  const response = await db
    ...
    .eq("id", id)
    .select(
      "id,estimate_amount,estimate_notes,approval_status,approval_token,approval_requested_at,approval_responded_at,approval_customer_name,approval_note",
    )
    .maybeSingle();
  if (response.error) {
    const message = workflowColumnsMissing(response.error)
      ? "Migrasi Priority 1-2-3 belum dijalankan. Buka folder supabase/migrations lalu jalankan SQL terbaru."
      : `Gagal membuat persetujuan: ${response.error.message || response.error}`;
    toast(message, "error");
    return;
  }
  ...
  await logWorkflowActivity(
    id,
    "approval_request",
    `Persetujuan estimasi ${rp(amount)} dikirim kepada pelanggan.`,
  );
  renderDash();
  openDetail(id);
  showMini(
    "Link persetujuan siap",
    "Salin link atau kirim langsung melalui WhatsApp pelanggan.",
    [
      { label: "Tutup", cls: "secondary", fn: null },
      { label: "Salin link", fn: () => copyApprovalLink(id) },
      ...(r.customer_phone
        ? [{ label: "Kirim WhatsApp", fn: () => sendApprovalWhatsApp(id) }]
        : []),
    ],
  );
}
...
async function resetEstimateApproval(id) {
  ...
  if (response.error) {
    toast(
      `Gagal membatalkan: ${response.error.message || response.error}`,
      "error",
    );
    return;
  }
...
async function renderWorkflowTimeline(reportId) {
  ...
  if (r.approval_status === "approved" || r.approval_status === "rejected") {
    const exists = events.some((event) =>
      String(event.action || "").includes(`approval_${r.approval_status}`),
    );
    if (!exists) {
      events.push({
        ...
        detail:
          r.approval_status === "approved"
            ? `Estimasi ${rp(r.estimate_amount || r.fee)} disetujui pelanggan.`
            : `Estimasi ditolak pelanggan${r.approval_note ? `: ${r.approval_note}` : "."}`,
  ...
  root.innerHTML = events
    .map((event) => {
      ...
      const symbol = action.includes("approved")
        ? "✓"
...
async function renderApproval(token) {
  ...
  if (!/^[0-9a-f-]{36}$/.test(safeToken)) {
    root.innerHTML =
      '<div class="pub-card"><h2>Link tidak valid</h2><p class="muted">Periksa kembali link persetujuan yang dikirim oleh toko.</p></div>';
    return;
  }
...
async function respondEstimateApproval(token, decision) {
  const name = (
    ($("approvalCustomerName") && $("approvalCustomerName").value) ||
    ""
  ).trim();
  const note = (
    ($("approvalCustomerNote") && $("approvalCustomerNote").value) ||
    ""
  ).trim();
  ...
  try {
    ...
    if (status)
      status.textContent = `Gagal menyimpan: ${(error && error.message) || error}`;
    ["approvalRejectBtn", "approvalApproveBtn"].forEach((id) => {
      const button = $(id);
