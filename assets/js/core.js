// ====== KONFIGURASI: semua nilai diisi di file config.js (index.html TIDAK perlu diubah per toko) ======
const CFG =
...
try {
  if (
    SUPABASE_URL &&
    !SUPABASE_URL.startsWith("MASUKKAN") &&
    SUPABASE_ANON_KEY &&
    !SUPABASE_ANON_KEY.startsWith("MASUKKAN")
  ) {
    // storage=sessionStorage → tiap TAB punya sesi login sendiri; bisa buka >1 akun di 1 browser tanpa saling menimpa/ketuker. storageKey tetap unik per toko. Catatan: sesi tidak bertahan setelah tab/browser ditutup — pakai biometrik utk masuk cepat.
...
const DEVICE_SPECS = {
  Laptop: [{ key: "seri", label: "Tipe / Seri", ph: "cth: X441UA" }],
  "PC/Komputer": [
    { key: "prosesor", label: "Prosesor", ph: "cth: Core i5-9400" },
    { key: "ram", label: "RAM", ph: "cth: 8GB" },
    { key: "storage", label: "Storage", ph: "cth: SSD 256GB" },
    { key: "vga", label: "VGA", ph: "cth: GTX 1650 / Onboard" },
  ],
  Printer: [
    {
      key: "tipe",
  ...
  "HP/Smartphone": [{ key: "imei", label: "IMEI / SN", ph: "opsional" }],
  CCTV: [
    { key: "jumlah", label: "Jumlah Kamera", ph: "cth: 4" },
    { key: "perekam", label: "DVR/NVR", ph: "cth: DVR 8CH" },
  ],
};
...
const LEVELS = {
  1: { name: "Ringan", color: "#22c55e" },
  2: { name: "Sedang", color: "#3b82f6" },
  3: { name: "Berat", color: "#f59e0b" },
  4: { name: "Sangat Berat", color: "#ef4444" },
};
...
// brand vendor dipakai saat paket TIDAK punya white-label (Basic)
const VENDOR_BRAND = {
...
// versi jual: semua fitur Pro aktif secara default (dikunci Pro). whitelabel dikontrol dari master panel.
...
const FEAT_LABELS = {
  ...
  collab: "Kolaborasi (Papan, komentar, notifikasi)",
...
function boardStages() {
  return Array.isArray(BOARD_STAGES) && BOARD_STAGES.length
    ? BOARD_STAGES
    : STAGES;
}
...
const fmtDate = (d) =>
  d
    ...
    : "-";
// ====== I18N (multi-bahasa: Indonesia & English US) ======
let LANG = "id";
try {
...
const I18N_EN = {
  ...
  "Nama ini muncul di komentar, aktivitas, & papan — bukan dari email.":
    "This name shows in comments, activity & board — not your email.",
  "Login sidik jari": "Fingerprint login",
  "Masuk cukup dengan biometrik di perangkat ini.":
    "Sign in with biometrics on this device.",
  ...
  "Logo PNG (opsional — untuk logo custom toko)":
    "PNG logo (optional — for custom store logo)",
  "Jika diisi, logo PNG dipakai menggantikan emoji.":
    "If set, the PNG logo replaces the emoji.",
  ...
  "Atur identitas toko dulu (bisa diubah lagi nanti di menu 🔒).":
    "Set up your store identity first (you can change it later in the 🔒 menu).",
  "Nanti saja": "Later",
  ...
  "Foto / Video Before (bisa lebih dari 1)":
    "Before Photos / Videos (multiple allowed)",
  "✅ AFTER (sesudah perbaikan)": "✅ AFTER (post-repair)",
  "Catatan hasil": "Result notes",
  "Foto / Video After (bisa lebih dari 1)":
    "After Photos / Videos (multiple allowed)",
  ...
  "Arahkan kamera lalu tekan Jepret. Bisa ambil banyak foto sekaligus, lalu tekan Selesai.":
    "Point the camera and tap Snap. Take multiple photos at once, then tap Done.",
  ...
  "Belum ada data. Tambahkan laporan pertamamu untuk melihat statistik.":
    "No data yet. Add your first report to see statistics.",
  ...
  "\ud83d\udcc8 Komponen Paling Sering":
    "\ud83d\udcc8 Most Frequent Components",
  "\ud83d\udcbb Merek Paling Sering": "\ud83d\udcbb Most Frequent Brands",
  "\ud83d\udda5\ufe0f Distribusi per Jenis Perangkat":
    "\ud83d\udda5\ufe0f Distribution by Device Type",
  ...
  "90 hari": "90 days",
  "3 hari": "3 days",
  "Level 1 - Ringan (software: install ulang OS, driver, aplikasi)":
    "Level 1 - Light (software: OS reinstall, drivers, apps)",
  "Level 2 - Sedang (bongkar & ganti part: LCD, keyboard, RAM/SSD, cleaning, repasta/thermal)":
    "Level 2 - Medium (open & replace parts: LCD, keyboard, RAM/SSD, cleaning, thermal repaste)",
  "Level 3 - Berat (engsel, port/konektor, jack charging, flexible)":
  ...
  "\ud83d\udccb Riwayat": "\ud83d\udccb History",
  "\ud83d\udcac Chat WA": "\ud83d\udcac WA Chat",
  "Belum ada pelanggan. Data muncul otomatis setelah kamu membuat laporan servis.":
    "No customers yet. Data appears automatically once you create a service report.",
  "Tanpa no. WhatsApp": "No WhatsApp number",
  "Direktori pelanggan otomatis dari riwayat servis. Klik":
...
function _translateTextNodes(root) {
  ...
  nodes.forEach((node) => {
    ...
    const key = (orig || "").trim();
    if (!key) return;
    if (LANG === "en" && I18N_EN[key] != null)
      node.nodeValue = orig.replace(key, I18N_EN[key]);
...
function _translatePlaceholders(root) {
  ...
  root.querySelectorAll("[placeholder]").forEach((el) => {
    if (!el.dataset.phOrig)
      el.dataset.phOrig = el.getAttribute("placeholder") || "";
    const o = el.dataset.phOrig;
    el.setAttribute(
      "placeholder",
      LANG === "en" && I18N_EN[o] != null ? I18N_EN[o] : o,
    );
  });
}
function applyLang() {
...
function brandLogoImg(px) {
  return BRAND.logoUrl
    ? `<img src="${esc(BRAND.logoUrl)}" alt="logo" style="height:${px}px;width:${px}px;object-fit:contain;border-radius:6px;vertical-align:middle" />`
    : "";
}
function applyBrand() {
  ...
  $("brandTitle").innerHTML =
    `${lg} ${esc(BRAND.name || "RepairLog")} <small>${esc(BRAND.tagline || "")}</small>`;
}
...
async function saveBrandFromForm() {
  const obj = {
    ...
    ticketFmtSvc:
      ($("brTicketSvc") && $("brTicketSvc").value.trim()) ||
      "RL/STTS/{BULAN}/{TAHUN}",
    ticketFmtWr:
      ($("brTicketWr") && $("brTicketWr").value.trim()) ||
      "STTS/{BULAN}/{TAHUN}",
...
async function uploadLogo(input) {
  ...
  try {
    ...
    await saveBrand({ logoUrl: data.publicUrl });
    const pv = $("brLogoPreview");
    if (pv)
      pv.innerHTML = `<img src="${esc(data.publicUrl)}" style="width:100%;height:100%;object-fit:contain" />`;
    $("brLogoStatus").textContent =
      "Logo terpasang ✓ (tersinkron ke semua perangkat).";
  } catch (e) {
...
async function uploadQris(input) {
  ...
  try {
    ...
    if (error) throw error;
    const { data } = db.storage.from("media").getPublicUrl(path);
    await saveBrand({ qrisUrl: data.publicUrl });
...
async function fetchConfig() {
  ...
  const { data } = await db
    ...
    .in("key", ["branding", "features", "board_stages"]);
  if (data) {
    data.forEach((r) => {
      if (r.key === "branding" && r.value)
        BRAND = { ...BRAND_DEFAULT, ...r.value };
      if (r.key === "features" && r.value)
        FEATURES = { ...FEATURES_DEFAULT, ...r.value };
      if (r.key === "board_stages" && Array.isArray(r.value) && r.value.length)
        BOARD_STAGES = r.value;
    });
  }
  if (LICENSE_FEATURES) {
  ...
  if (!FEATURES.whitelabel) {
    BRAND = { ...VENDOR_BRAND };
  } // paket tanpa white-label → kunci ke brand vendor
  applyBrand();
  applyFeatures();
}
...
async function checkStorageWarn() {
  try {
    ...
    const { data } = await db.rpc("my_storage_usage");
    const used = Number(data) || 0;
    const limit = LICENSE_STORAGE_MB * 1048576;
    if (limit > 0 && used > limit)
      toast(
        "Storage toko melebihi jatah (" +
          Math.round(used / 1048576) +
          "/" +
          LICENSE_STORAGE_MB +
          " MB). Hubungi admin untuk menambah kapasitas.",
        "error",
      );
  } catch (e) {}
}
function applyFeatures() {
  ...
  $("chartsWrap").style.display =
    FEATURES.dashboard && FEATURES.charts ? "" : "none";
  $("phoneWrap").style.display = FEATURES.whatsapp ? "" : "none";
  $("bioSetRow").style.display = FEATURES.biometric ? "" : "none";
  if ($("navBoard"))
    $("navBoard").style.display = FEATURES.collab ? "" : "none";
  if ($("notifBtn"))
    $("notifBtn").style.display = FEATURES.collab ? "" : "none";
  if ($("navAttend"))
    $("navAttend").style.display = FEATURES.attendance ? "" : "none";
...
// ====== KUNCI TOKO (isolasi antar-toko pada 1 project Supabase bersama) ======
// Tiap akun terikat ke SATU toko (profiles.store_id). Kalau akun dipakai di toko lain, login ditolak.
...
function buildCompChecks(type) {
  ...
  $("compChecks").innerHTML = list
    .map(
      (c) =>
        `<label class="chk"><input type="checkbox" value="${esc(c)}" onchange="onCompToggle()"> ${esc(c)}</label>`,
    )
    .join("");
}
...
function buildKelengkapan(sel, isGaransi) {
  ...
  let html = opts
    .map(
      (c) =>
        `<label class="chk"><input type="checkbox" value="${esc(c)}"${sel.includes(c) ? " checked" : ""}> ${esc(c)}</label>`,
    )
    .join("");
...
function getKelengkapan() {
  const arr = [...document.querySelectorAll("#kelengkapanChecks input:checked")]
    .filter((i) => i.id !== "klotherchk")
    .map((i) => i.value);
  const c = $("klotherchk");
  const oi = $("klOther");
...
function getPayMeta() {
  ...
  if (m === "Split Bill") {
    ...
    o.transfer =
      parseRupiah($("f_splittransfer") ? $("f_splittransfer").value : "") || 0;
  }
  return o;
}
function togglePaySplit() {
  ...
  if (w)
    w.style.display =
      ($("f_paymethod") && $("f_paymethod").value) === "Split Bill"
        ? ""
        : "none";
}
...
async function saveClaim() {
  ...
  const { error } = await db
    .from("reports")
    .update({ device_specs: specs, updated_at: new Date().toISOString() })
    .eq("id", id);
  if (error) {
...
function buildDevTypeOptions() {
  const el = $("f_devtype");
  if (el)
    el.innerHTML = DEVICE_TYPES.map((t) => `<option>${esc(t)}</option>`).join(
      "",
    );
}
...
function buildAssignOptions(sel) {
  ...
  el.innerHTML =
    ...
    TEAM.map(
      (t) =>
        `<option value="${t.user_id}"${sel === t.user_id ? " selected" : ""}>${esc(t.name || t.email)}</option>`,
    ).join("");
}
function getSelectedComps() {
  return [...document.querySelectorAll("#compChecks input:checked")].map(
    (i) => i.value,
  );
}
...
function buildTicketTail(job) {
  ...
  const tpl = isWr
    ? BRAND.ticketFmtWr || "STTS/{BULAN}/{TAHUN}"
    : BRAND.ticketFmtSvc || "RL/STTS/{BULAN}/{TAHUN}";
  return String(tpl)
...
async function genTicket() {
  ...
  try {
    if (db) {
      const { data, error } = await db.rpc("next_ticket_seq", {
        p_store_id: STORE_ID,
        p_period: period,
      });
...
// ====== ZOOM PAPAN (mobile: tombol + cubit/pinch) ======
let _boardZoom = 1;
...
function setBoardZoom(z) {
  _boardZoom = Math.min(1.8, Math.max(0.5, z));
  applyBoardZoom();
}
function boardZoomBtn(d) {
  setBoardZoom(Math.round((_boardZoom + d * 0.1) * 10) / 10);
}
(function () {
...
// ====== ZOOM LAPORAN (tombol +/- untuk daftar laporan) ======
let _listZoom = 1;
  ...
  if (w) {
    try {
      ...
      w.style.gridTemplateColumns =
        "repeat(auto-fill,minmax(" + _min + "px,1fr))";
    } catch (e) {}
  }
  const l = $("listZoomLbl");
...
function setListZoom(z) {
  _listZoom = Math.min(1.6, Math.max(0.6, z));
  applyListZoom();
}
function listZoomBtn(d) {
  setListZoom(Math.round((_listZoom + d * 0.1) * 10) / 10);
}
...
// ====== ZOOM TABEL (mobile: tombol +/- & cubit/pinch) untuk Pelanggan, Keuangan, Absensi ======
const _ZOOM_BOX = {
  cust: "custBox",
  finance: "financeBox",
  attend: "attendBox",
};
const _zoomVal = { cust: 1, finance: 1, attend: 1 };
...
function setTblZoom(key, z) {
  _zoomVal[key] = Math.min(1.8, Math.max(0.5, z));
  applyTblZoom(key);
}
function zoomBtn(key, d) {
  setTblZoom(key, Math.round((_zoomVal[key] + d * 0.1) * 10) / 10);
}
