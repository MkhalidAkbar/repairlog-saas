function openSupplierForm(id) {
  ...
  $("supplierFormTitle").textContent = supplier
    ? "Edit Supplier"
    : "Tambah Supplier";
...
async function saveSupplier() {
  ...
  const result = id
    ? await db.from("suppliers").update(payload).eq("id", id)
...
function openPurchaseOrderForm(id, supplierId) {
  ...
  PO_ITEMS_STATE = Array.isArray(order?.items)
    ? order.items.map((item) => ({ ...item }))
    : [{ part_id: "", name: "", qty: 1, unit_cost: 0 }];
  $("poFormTitle").textContent = order
...
function addPurchaseOrderItem() {
  PO_ITEMS_STATE.push({ part_id: "", name: "", qty: 1, unit_cost: 0 });
  renderPurchaseOrderItems();
}

function removePurchaseOrderItem(index) {
  PO_ITEMS_STATE.splice(index, 1);
  if (!PO_ITEMS_STATE.length) addPurchaseOrderItem();
  else renderPurchaseOrderItems();
}
...
function renderPurchaseOrderItems() {
  const root = $("poItems");
  if (!root) return;
  const options = PARTS.map(
    (part) => `<option value="${part.id}">${esc(part.name)}</option>`,
  ).join("");
...
function renderPurchaseOrderTotal() {
  const total = PO_ITEMS_STATE.reduce(
    (sum, item) =>
  ...
  if ($("poTotal"))
    $("poTotal").innerHTML =
      `<span>Total PO</span><strong>${rp(total)}</strong>`;
}

async function savePurchaseOrder(status) {
  const supplierId = $("po_supplier").value;
  const items = PO_ITEMS_STATE.filter(
    (item) => item.part_id && (Number(item.qty) || 0) > 0,
  );
  ...
  const total = items.reduce(
    (sum, item) => sum + Number(item.qty) * Number(item.unit_cost || 0),
    0,
  );
  const payload = {
    store_id: STORE_ID,
  ...
  const result = id
    ? await db.from("purchase_orders").update(payload).eq("id", id)
    : await db.from("purchase_orders").insert(payload);
  if (result.error) {
  ...
  toast(
    status === "ordered" ? "Purchase order dibuat." : "Draft PO tersimpan.",
    "success",
  );
}
...
function receivePurchaseOrder(id) {
  ...
  showMini(
    ...
    `Tambahkan seluruh item ${order.order_no} ke stok?`,
    [
...
function openStockMovementForm(partId, type) {
  ...
  $("sm_part").innerHTML =
    ...
    PARTS.map(
      (part) =>
        `<option value="${part.id}">${esc(part.name)} • stok ${Number(part.stock) || 0}</option>`,
    ).join("");
...
function updateStockMovementHint() {
  ...
  if ($("sm_qty_label"))
    $("sm_qty_label").textContent =
      type === "adjustment" ? "Stok fisik baru" : "Jumlah";
  if ($("sm_hint"))
    $("sm_hint").textContent =
      type === "adjustment"
        ? "Masukkan jumlah stok fisik hasil perhitungan, bukan selisihnya."
        : type === "return"
          ? "Jumlah akan dikurangi dari stok dan dicatat sebagai retur."
