// ====== REPAIRLOG v3.3 BUSINESS DATA CORE ======
let SUPPLIERS = [];
let PURCHASE_ORDERS = [];
let STOCK_MOVEMENTS = [];
let WARRANTY_CLAIMS = [];
...
function businessSchemaMissing(error) {
  ...
  return (
    ...
    /does not exist|not found|schema cache|original_report_id|warranty_claims|purchase_orders|stock_movements|suppliers/i.test(
      message,
    )
  );
}
...
function supplierName(id) {
  return (
    SUPPLIERS.find((supplier) => String(supplier.id) === String(id))?.name ||
    "-"
  );
}
...
async function loadBusinessSuiteData() {
  ...
  const configs = [
    ["suppliers", "name", true],
    ["purchase_orders", "created_at", false],
    ["stock_movements", "created_at", false],
    ["warranty_claims", "created_at", false],
  ];
  const targets = [
    SUPPLIERS,
    PURCHASE_ORDERS,
  ...
  try {
    const links = await db
      .from("reports")
      .select(
        "id,original_report_id,warranty_claim_id,claim_sequence,stock_finalized_at",
      )
      .eq("store_id", STORE_ID);
    if (links.error) {
