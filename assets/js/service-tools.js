// ====== PRIORITY 4–6: WHATSAPP AUTOMATION, QR TICKET & QUALITY CONTROL ======
const PRIORITY456_COLUMNS = [
  "wa_automation_state",
  "wa_next_reminder_at",
  "wa_last_sent_at",
  "wa_last_event",
...
const QC_MANUAL_ITEMS = [
  ["power", "Perangkat menyala dan mati dengan normal"],
  ["primary_function", "Fungsi utama perangkat sudah diuji"],
  ["complaint_resolved", "Keluhan awal sudah ditangani"],
  ["physical", "Kondisi fisik dan kebersihan diperiksa"],
  ["accessories", "Kelengkapan pelanggan sudah dicocokkan"],
];
...
function priority456MigrationReady(report) {
  return Boolean(
    report && Object.prototype.hasOwnProperty.call(report, "qc_status"),
  );
}

function ticketPublicLink(report) {
  if (!report) return "";
  if (typeof trackOrWarrantyLink === "function")
    return trackOrWarrantyLink(report);
...
function whatsappMessage(report, eventKey) {
  const template =
    WA_AUTOMATION_TEMPLATES[eventKey] || WA_AUTOMATION_TEMPLATES.received;
  const end = warrantyEndDate(report);
  return template.message
    .replace(/{nama}/g, report.customer || "Kak")
    .replace(/{device}/g, report.device || "perangkat Anda")
    .replace(/{tiket}/g, report.ticket_no || "-")
    .replace(
      /{toko}/g,
      (typeof BRAND !== "undefined" && BRAND.name) || "RepairLog",
    )
    .replace(/{link}/g, ticketPublicLink(report))
    .replace(/{approval_link}/g, approvalPublicLink(report))
    .replace(/{payment_status}/g, report.payment_status || "Belum")
    .replace(
      /{total}/g,
      typeof rp === "function" ? rp(report.fee || 0) : String(report.fee || 0),
    )
    .replace(
      /{warranty_end}/g,
      end
...
function recommendedWhatsAppEvent(report) {
  ...
  const done =
    status.includes("selesai") || stage === "selesai" || stage === "diambil";
  if (approval === "pending" && !sent("approval")) return "approval";
  if (done && !sent("completed")) return "completed";
  if (
    done &&
    String(report.payment_status || "").toLowerCase() !== "lunas" &&
    !sent("payment")
  )
    return "payment";
  if (stage === "selesai" && !report.date_out && !sent("pickup"))
    return "pickup";
  const warrantyEnd = warrantyEndDate(report);
  if (warrantyEnd) {
    const days = Math.ceil((warrantyEnd.getTime() - Date.now()) / 86400000);
    if (days >= 0 && days <= 7 && !sent("warranty")) return "warranty";
  }
  if (
    (stage.includes("dikerjakan") || status === "proses") &&
    !sent("progress")
  )
    return "progress";
...
async function persistPriority456Report(id, patch, silent) {
  ...
  if (result.error) {
    if (!silent) {
      toast(
        priority456ColumnsMissing(result.error)
          ? "Jalankan migrasi Priority 4-5-6 terlebih dahulu."
          : `Gagal menyimpan: ${result.error.message || result.error}`,
        "error",
      );
    }
    return false;
...
async function sendWhatsAppAutomation(id, eventKey, customMessage) {
  ...
  if (!report) return;
  const number =
    typeof waNumber === "function"
      ? waNumber(report.customer_phone)
      : String(report.customer_phone || "").replace(/\D/g, "");
  if (!number) {
  ...
  window.open(
    `${waBase}${number}?text=${encodeURIComponent(message)}`,
    "whatsapp",
  );

  const now = new Date().toISOString();
  const state = whatsappState(report);
  state[key] = { sent_at: now, message };
  const patch = {
  ...
  if (!saved) {
    ...
    toast(
      "WhatsApp dibuka. Riwayat belum tersimpan—jalankan migrasi 4-5-6.",
      "error",
    );
  } else {
    if (typeof logWorkflowActivity === "function") {
      await logWorkflowActivity(
        id,
        "whatsapp",
        `Pesan WhatsApp “${WA_AUTOMATION_TEMPLATES[key].label}” dibuka untuk dikirim.`,
      );
    }
...
function editAndSendWhatsApp(id, eventKey) {
  ...
  const key = WA_AUTOMATION_TEMPLATES[eventKey] ? eventKey : "received";
  showPrompt(
    `WhatsApp — ${WA_AUTOMATION_TEMPLATES[key].label}`,
    "Periksa atau edit pesan sebelum dikirim:",
    "Isi pesan WhatsApp",
    whatsappMessage(report, key),
    (value) => {
...
function selectedWhatsAppTemplate(id) {
  ...
  return field && WA_AUTOMATION_TEMPLATES[field.value]
    ? field.value
    : recommendedWhatsAppEvent(priority456Report(id)) || "received";
}
...
async function scheduleWhatsAppReminder(id) {
  ...
  if (typeof logWorkflowActivity === "function") {
    await logWorkflowActivity(
      id,
      "whatsapp_reminder",
      `Pengingat WhatsApp dijadwalkan ${date.toLocaleString("id-ID")}.`,
    );
  }
...
function whatsappActionItems(source) {
  ...
  (Array.isArray(source) ? source : reports).forEach((report) => {
    ...
    const reminder = report.wa_next_reminder_at
      ? new Date(report.wa_next_reminder_at)
      : null;
    if (
      reminder &&
    ...
    if (
      key &&
      ["approval", "completed", "payment", "pickup", "warranty"].includes(key)
    ) {
      items.push({
...
function qcEvidence(report) {
  return {
    after_photo:
      Array.isArray(report && report.after_media) &&
      report.after_media.length > 0,
...
function ensureQualityControlBeforeFinish(id, target) {
  ...
  const report = priority456Report(id);
  // Instalasi lama tetap dapat dipakai sampai migrasi 4-5-6 dijalankan.
  if (report && !priority456MigrationReady(report)) return true;
  if (report && qualityControlPassed(report)) return true;
  toast(
    "Selesaikan dan luluskan Quality Control sebelum menutup tiket.",
    "error",
  );
  if (report) {
...
async function saveQualityControl(id, complete) {
  ...
  const checklist = qcChecklist({ ...report, qc_items: items });
  const missing = checklist.filter((item) => !item.checked);
  if (complete && missing.length) {
    toast(
      `QC belum lengkap: ${missing.map((item) => item.label).join(", ")}.`,
      "error",
    );
    return;
  }
  ...
  if (typeof logWorkflowActivity === "function") {
    await logWorkflowActivity(
      ...
      "quality_control",
      complete
        ? "Quality Control dinyatakan lulus."
        : "Draft Quality Control disimpan.",
    );
  }
  toast(
    complete
      ? "Quality Control lulus. Tiket dapat diselesaikan."
      : "Draft QC disimpan.",
    "success",
  );
  renderPriority456Detail(id);
...
async function reopenQualityControl(id) {
  ...
  if (typeof logWorkflowActivity === "function")
    await logWorkflowActivity(
      id,
      "quality_control",
      "Quality Control dibuka kembali untuk pemeriksaan ulang.",
    );
  renderPriority456Detail(id);
}
...
function qualityControlActionItems(source) {
  ...
  (Array.isArray(source) ? source : reports).forEach((report) => {
    ...
    const needsQc =
      stage.includes("qc") ||
      stage.includes("testing") ||
      (qualityControlTarget(report.stage || report.status) &&
        !qualityControlPassed(report));
...
function renderWhatsAppAutomationCard(report) {
  ...
  const state = whatsappState(report);
  const reminderValue =
    report.wa_next_reminder_at && typeof toLocalDateTimeValue === "function"
      ? toLocalDateTimeValue(report.wa_next_reminder_at)
      : "";
  const options = Object.entries(WA_AUTOMATION_TEMPLATES)
    .map(
      ([key, item]) =>
        `<option value="${key}"${key === recommended ? " selected" : ""}>${esc(item.label)}${state[key] && state[key].sent_at ? " • terkirim" : ""}</option>`,
    )
    .join("");
  const lastSent = report.wa_last_sent_at
    ? workflowDateTime(report.wa_last_sent_at)
...
function renderQualityControlCard(report) {
  ...
  const statusLabel = passed
    ? "Lulus"
    : String(report.qc_status || "not_started") === "in_progress"
      ? "Sedang diperiksa"
      : "Belum dimulai";
  const checks = progress.items
    .map(
      (item) =>
        `<label class="qc-item ${item.auto ? "qc-auto" : ""} ${item.checked ? "is-checked" : ""}"><input type="checkbox" data-qc-key="${item.key}"${item.checked ? " checked" : ""}${item.auto ? " disabled" : ""}><span class="qc-check" aria-hidden="true">${item.checked ? "✓" : ""}</span><span><strong>${esc(item.label)}</strong>${item.auto ? `<small>${item.checked ? "Terverifikasi dari data tiket" : "Lengkapi data tiket terlebih dahulu"}</small>` : ""}</span></label>`,
    )
    .join("");
...
function renderPriority456Detail(reportId) {
  ...
  panel.innerHTML = `<div class="service-tools-head"><div><span class="dashboard-kicker">Komunikasi & penyelesaian</span><h3>WhatsApp, QR tiket, dan Quality Control</h3></div><span class="service-tools-version">Prioritas 4–6</span></div><div class="service-tools-grid">${renderWhatsAppAutomationCard(report)}${renderTicketQrCard(report)}${renderQualityControlCard(report)}</div>`;
  const actions = [...container.children]
    .reverse()
    .find(
      (element) => element.classList && element.classList.contains("actions"),
    );
  if (actions) container.insertBefore(panel, actions);
