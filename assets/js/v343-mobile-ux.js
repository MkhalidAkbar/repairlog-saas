(() => {
    ...
    let printerDevice = null, printerCharacteristic = null, pendingReceiptId = null;
    const allReports = () => typeof reports !== "undefined" ? reports : [];
    const getReport = id => allReports().find(report => String(report.id) === String(id));
    ...
    function syncPrinterSettings() {
        ...
        if (printerDevice?.gatt?.connected && printerCharacteristic) setPrinterStatus(`${printerDevice.name || "Printer thermal"} siap mencetak.`, true);
    }
    async function writableCharacteristic(device) {
        ...
        for (const uuid of SERVICE_UUIDS) {
            try {
                ...
                const writable = chars.find(item => item.properties.writeWithoutResponse || item.properties.write || item.properties.writeWithResponse);
    ...
    async function connectPrinter(askUser = true) {
        ...
        if (!printerDevice && !askUser && navigator.bluetooth.getDevices) {
            ...
            printerDevice = known.find(device => device.gatt) || null;
        }
        if (!printerDevice) {
            printerDevice = await navigator.bluetooth.requestDevice({ acceptAllDevices: true, optionalServices: SERVICE_UUIDS });
    ...
    async function pairThermalPrinterV343() {
        ...
        setPrinterStatus("Mencari printer Bluetooth…");
        try {
            await connectPrinter(true);
            if (typeof toast === "function") toast("Printer thermal tersambung.", "success");
        } catch (error) {
            if (error?.name === "NotFoundError") setPrinterStatus("Pemilihan printer dibatalkan.");
            else {
    ...
    function receiptLines(report) {
        ...
        items.forEach(item => {
            const qty = Number(item.qty) || 1, unit = Number(item.price) || 0;
        ...
        if (report.tasks || serviceTotal || !lines.length) lines.unshift({ label: report.tasks ? `Service (${report.tasks})` : "Service", qty: 1, unit: serviceTotal || total, total: serviceTotal || total });
        return lines;
    }
    function renderReceipt(report) {
        const lines = receiptLines(report), subtotal = lines.reduce((sum, line) => sum + line.total, 0), total = Number(report.fee) || subtotal;
        ...
        if (!area) return;
        area.innerHTML = `<article class="pickup-receipt-v342">${logo}<h1>${esc(BRAND.name || "RepairLog")}</h1><p class="pickup-receipt-address">${esc(BRAND.tagline || "Service & Sparepart")}</p><div class="pickup-receipt-rule"></div><div class="pickup-receipt-meta"><span>No. tiket</span><strong>${esc(report.ticket_no || "-")}</strong><span>Tanggal ambil</span><strong>${fmtDate(report.date_out || (new Date).toISOString())}</strong><span>Customer</span><strong>${esc(report.customer || "-")}</strong><span>Perangkat</span><strong>${esc(report.device || "-")}</strong><span>Kasir / Teknisi</span><strong>${esc(typeof techName === "function" ? techName(report.assigned_to) : "-")}</strong></div><div class="pickup-receipt-rule"></div>${rows}<div class="pickup-receipt-rule"></div><div class="pickup-receipt-total"><span>Sub total</span><strong>${rp(subtotal)}</strong><span>Total</span><strong>${rp(total)}</strong>${method ? `<span>Bayar (${esc(method)})</span><strong>${rp(total)}</strong>` : ""}<span>Status</span><strong>${esc(report.payment_status || "Belum")}</strong></div><div class="pickup-receipt-rule"></div><h2>TERIMA KASIH</h2><p class="pickup-receipt-note">Struk ini menjadi bukti bahwa customer sudah mengambil barang. Mohon lakukan pengecekan sebelum meninggalkan toko.</p><p class="pickup-receipt-time">Dicetak ${(new Date).toLocaleString("id-ID")}</p></article>`;
    }
    function ascii(value) { return String(value ?? "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^\x20-\x7E\n]/g, " ").replace(/\s+/g, " ").trim(); }
    function fit(value, width) { const text = ascii(value); return text.length > width ? `${text.slice(0, Math.max(1, width - 1))}~` : text; }
    function pair(left, right, width = 42) { const r = fit(right, Math.floor(width * .48)), l = fit(left, Math.max(1, width - r.length - 1)); return l + " ".repeat(Math.max(1, width - l.length - r.length)) + r; }
    function receiptEscPos(report) {
        const width = 42, items = receiptLines(report), total = Number(report.fee) || items.reduce((sum, item) => sum + item.total, 0), divider = "-".repeat(width), text = [];
        text.push(ascii(BRAND.name || "RepairLog")); if (BRAND.tagline) text.push(ascii(BRAND.tagline));
        text.push(divider, pair("No. tiket", report.ticket_no || "-", width), pair("Tanggal ambil", fmtDate(report.date_out || (new Date).toISOString()), width), pair("Customer", report.customer || "-", width), pair("Perangkat", report.device || "-", width), divider);
        items.forEach(item => { text.push(fit(item.label, width), pair(`${item.qty} x ${rp(item.unit)}`, rp(item.total), width)); });
        text.push(divider, pair("TOTAL", rp(total), width), pair("Status", report.payment_status || "Belum", width), divider, "TERIMA KASIH", "Simpan struk ini sebagai bukti pengambilan barang.");
        const encoder = new TextEncoder, chunks = [new Uint8Array([27,64,27,97,1,27,69,1]),encoder.encode(text.slice(0,2).join("\n")+"\n"),new Uint8Array([27,69,0,27,97,0]),encoder.encode(text.slice(2,-3).join("\n")+"\n"),new Uint8Array([27,97,1,27,69,1]),encoder.encode(text.slice(-3).join("\n")+"\n\n\n"),new Uint8Array([27,69,0,29,86,66,0])];
    ...
    async function writePrinter(bytes) {
        ...
        for (let offset = 0; offset < bytes.length; offset += 20) {
            ...
            else if (typeof characteristic.writeValueWithResponse === "function") await characteristic.writeValueWithResponse(chunk);
    ...
    function openReceiptPrintOptions(id) {
        ...
        if (!report || !(report.stage === "Diambil" || /diambil/i.test(report.status || ""))) return typeof toast === "function" && toast("Cetak resi tersedia setelah tiket masuk papan Diambil.", "error");
        pendingReceiptId = id; renderReceipt(report); ensurePrintModal();
        const bluetooth = document.getElementById("receiptBluetoothOption"), hint = document.getElementById("receiptPrintHint");
        if (bluetooth) bluetooth.style.display = supportsBluetoothPrint() ? "grid" : "none";
        if (hint) hint.textContent = supportsBluetoothPrint() ? "Nyalakan printer, lalu pilih Printer Bluetooth. Koneksi pertama akan meminta izin perangkat." : "Browser ini tidak membuka printer Bluetooth langsung. Gunakan Dialog cetak; pada iPhone pilih printer AirPrint yang tersedia.";
        openModal("receiptPrintModal");
    }
    function printReceiptSystemV343() {
        const report = getReport(pendingReceiptId); if (!report) return; renderReceipt(report); closeModal("receiptPrintModal");
        ...
        document.getElementById("printArea")?.getBoundingClientRect(); window.print();
    }
    async function printReceiptBluetoothV343() {
        const report = getReport(pendingReceiptId), button = document.getElementById("receiptBluetoothOption"); if (!report || !button) return; button.disabled = true;
        try { await writePrinter(receiptEscPos(report)); closeModal("receiptPrintModal"); if (typeof toast === "function") toast("Resi berhasil dikirim ke printer thermal.", "success"); }
        catch (error) { if (error?.name !== "NotFoundError" && typeof toast === "function") toast(error.message || "Gagal mencetak ke printer Bluetooth.", "error"); }
        finally { button.disabled = false; }
    }
    const previousOpenSettings = window.openSettings;
    if (previousOpenSettings) window.openSettings = function openSettingsV343() { const result = previousOpenSettings(); syncPrinterSettings(); return result; };
    ["setCrmV34","setFinanceV34","setHealthV34","setFinanceKindV34"].forEach(name => {
        const original = window[name]; if (!original) return;
        window[name] = function scrollableTabsV343(...args) {
            const result = original.apply(this, args);
            requestAnimationFrame(() => { const root = name.includes("Crm") ? document.getElementById("crmTabs") : name.includes("Finance") ? document.getElementById("finTabs") : document.querySelector("#tab-health .rl-tabs"); root?.querySelector("button.active")?.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"}); });
            return result;
        };
    });
    ...
    window.printPickupReceiptV342 = id => { if (isMobile()) return openReceiptPrintOptions(id); pendingReceiptId = id; return printReceiptSystemV343(); };
    ensurePrintModal(); syncPrinterSettings(); if (typeof applyBiometricUi === "function") applyBiometricUi();
})();
